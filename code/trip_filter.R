...
files <- list.files(path = paste0(WD, "GitData/Bird-borne-radar-detection/output/"), pattern = "*trips_L1.csv", recursive = TRUE)
...
#for (i in seq_along(colonysites)){
  ...
  # write dataset
  fwrite(dataGroup, file=paste0(WD,"GitData/Bird-borne-radar-detection//output/",colonysites[i],"_trips_L2.csv"),row.names=FALSE)
